...
- **Использовать LSP для семантической верификации.** Когда нужно подтвердить наличие/отсутствие символа, использования функции, контракта API — использовать LSP-операции (`workspaceSymbol`, `documentSymbol`, `findReferences`, `goToDefinition`, `outgoingCalls`, `hover`), не текстовый grep. LSP смотрит на AST/индекс, поэтому видит реальный экспорт/импорт/вызов, а не текстовое совпадение (grep ловит и комментарии, и import-имена, и строки в тестах). Grep остаётся только для проверки **отсутствия** конкретного текстового шаблона в одном файле, где LSP не даёт прямого ответа (например, что в файле нет вхождения `metadataCache.set(`). Read — для инспекции тела функции или дифф-сверки. Это правило применяется и в финальной верификации планов («Соответствует ли результат критериям готовности?»), и в любых ad-hoc проверках («Где используется X?», «Существует ли символ Y?»).
