...
from app.enrichers.pdf_llm import extract_metadata_from_filename, LlmMetadata


def test_empty_api_key_returns_empty():
    with patch("app.enrichers.pdf_llm.ANTHROPIC_API_KEY", ""):
...
def test_extraction_parses_json_response():
    ...
    with patch("app.enrichers.pdf_llm.ANTHROPIC_API_KEY", "test-key"), \
         patch("app.enrichers.pdf_llm.anthropic.Anthropic") as mock_client:
