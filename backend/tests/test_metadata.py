...
class TestMetadataSearch:
    def test_empty_query(self, reader_client):
        resp = reader_client.get("/api/metadata/search", params={"q": ""})
    ...
    def test_auth_required(self, client):
        resp = client.get("/api/metadata/search", params={"q": "test"})
    ...
    def test_provider_error_returns_empty(self, mock_search, reader_client):
        ...
        resp = reader_client.get("/api/metadata/search", params={"q": "test book"})
        assert resp.status_code == 200
        assert resp.json() == {"results": []}
    ...
    def test_happy_path(self, mock_search, reader_client):
        ...
        resp = reader_client.get("/api/metadata/search", params={"q": "test"})
        assert resp.status_code == 200
        data = resp.json()
...
class TestCoverProxy:
    def test_auth_required(self, client):
        resp = client.get("/api/metadata/cover-proxy", params={"url": "https://cv5.litres.ru/img.jpg"})
        assert resp.status_code == 401

    def test_empty_url(self, reader_client):
        resp = reader_client.get("/api/metadata/cover-proxy")
        assert resp.status_code == 400

    def test_ftp_scheme_rejected(self, reader_client):
        resp = reader_client.get("/api/metadata/cover-proxy", params={"url": "ftp://cv5.litres.ru/img.jpg"})
        assert resp.status_code == 400

    def test_file_scheme_rejected(self, reader_client):
        resp = reader_client.get("/api/metadata/cover-proxy", params={"url": "file:///etc/passwd"})
        assert resp.status_code == 400

    def test_non_whitelist_domain_rejected(self, reader_client):
        resp = reader_client.get("/api/metadata/cover-proxy", params={"url": "https://evil.com/img.jpg"})
        assert resp.status_code == 403

    def test_localhost_rejected(self, reader_client):
        resp = reader_client.get("/api/metadata/cover-proxy", params={"url": "http://localhost:8080/secret"})
        assert resp.status_code == 403
