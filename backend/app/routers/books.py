...
def update_book(
    ...
    # Снимок детали строится в сервисе для события; клиенту тело не нужно —
    # применение приходит серверным SSE-событием.
    book_service.update_book(db, book_id, body, user.user_id)
