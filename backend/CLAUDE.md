...
- **Не глотать schema-ошибки БД.** `except sqlite3.OperationalError` с проверкой
  `"no such column"` / `"no such table"` и тихой деградацией (skip, `return None`,
  legacy-режим) — **запрещён**. Схема из `schema.sql` — источник истины: колонка/таблица
  обязана существовать, её отсутствие — баг схемы, который должен падать громко, а не
  маскироваться под «пусто». Обвязывать код вокруг отсутствия колонок/таблиц в БД —
  прямой запрет Alexey. Вычищенный пример анти-паттерна: `token_epoch` legacy-mode в
  `auth.py` (удалён в 38w.1). Легитимные миграции схемы/данных — идемпотентные скрипты
  в `scripts/` (прогоняются на проде **до** деплоя, если новый код строже старого), а
  не рантайм-обвязка в коде приложения.
