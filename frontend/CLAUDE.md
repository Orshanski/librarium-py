...
- **Производное состояние из свойства на странице-карточке сущности и ложно
  ...
  книг серии.
- **Подписывать систему отсчёта у чисел, пересекающих границу компонента.**
  Величина, у которой есть выбор системы отсчёта — индекс от нуля против номера
  от единицы, доля против количества, «до текущего элемента» против «включая
  его», — сопровождается однострочной подписью прямо в типе props или в поле
  detail коллбэка. Без подписи два соседних компонента расходятся молча: ошибка
  не ломает сборку, не роняет тест и проявляется дрейфом на единицу, который
  списывают на что угодно. Кейс (l35l): `pdf-nav-bar.tsx` свои свойства
  подписывал индексом от нуля и потому читался однозначно, а `pdf-reader.tsx`
  отдавал наружу номер от единицы без подписи — каждое открытие PDF сдвигало
  сохранённое место на страницу вперёд, и номер страницы в панели был на единицу
  больше настоящего. Подпись ставится в момент, когда число покидает компонент,
  а не когда о нём спотыкается следующий.
- **Тест, подменяющий величину, не проверяет её систему отсчёта.** Подменяя
  `offsetLeft`, `getBoundingClientRect` или иную величину раскладки, тест
  записывает в фикстуру ту систему отсчёта, которую предполагает автор, — и
  ошибка в самой системе отсчёта остаётся зелёной при любом прогоне. Поэтому
  правки, меняющие точку отсчёта (offsetParent, координатное пространство,
  единицы), принимаются глазами в браузере, и это говорится вслух в отчёте, а
  не подразумевается. Кейс (9euy): лента разделов на телефоне считала положение
  пункта через `offsetLeft`, а началом отсчёта была закреплённая обёртка —
  единственный позиционированный предок, — и к прокрутке прибавлялся её боковой
  отступ в 16 px; тест подменял тот же `offsetLeft` и промах не видел. Лечится
  тем, что контейнер объявляется началом отсчёта для своих детей
  (`position: relative`), но доказывается только на устройстве.
