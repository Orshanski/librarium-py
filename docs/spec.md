...
- The backend publication stream is the only source of domain events on the bus: the frontend no longer publishes server-type events locally, and the initiating client applies its own mutation through the same SSE event as every other client (`offlineBookChanged` stays client-only). `bookUpdated` from a book edit carries a `detail` snapshot (`{book, files, identifiers}`, user-scoped fields stripped) which the handler merges into an existing `book/{id}` cache entry — never replacing it wholesale and never creating one.
- Backend mutations publish domain events after DB commit via `/api/events/stream` (`EventSource`, `event: domain`). The publication point first persists the exact wire envelope in `sse_publications`, then wakes active stream readers; no live-only message is delivered without a durable row.
